...
export const POST = async (req: Request,) => {
  ...
  const { item, supplier, lot, invoice, inspector, rir, hbOne, hbTwo, hbThree, images, id, imagesOldName, originalInspector } = body;
  const product = `CH.${item}`;
  let imagesName = [];

  try {
    for (let i = 0; imagesOldName.length > i; i++) {
      fs.unlinkSync(`/home/alisson/Documents/imagens/chapas/${imagesOldName[i]}`);
    }
    ...
    const plate = await db.plate.update({
      where: {
        id,
      },
      data: {
