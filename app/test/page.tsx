...
import { ChevronDown, Printer, Loader2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function ListaEurocard() {
  ...
  const fetchModulos = async (page: number) => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/register/eurocard/list?page=${page}&limit=20`);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 md:p-8 mb-12">
      ...
      <div className="print:hidden mb-6">
        ...
        <p className="text-zinc-500 text-sm">Histórico e visualização de medições de atenuação por canal (20 por página).</p>
      </div>
      ...
      {loading ? (
        ...
        <>
                  ...
                  <div className="border-2 border-zinc-900 text-sm w-full max-w-4xl mx-auto print:max-w-full">
                    ...
                    {eurocardRows.map((row, index) => {
                      ...
                      return (
                        <div
                          key={row.id}
                          className={`grid grid-cols-12 divide-x-2 divide-zinc-900 ${
                            index !== eurocardRows.length - 1 ? 'border-b-2 border-zinc-900' : ''
                          }`}
                        >
          ...
          {/* Barra de Paginação (Ocultada na Impressão) */}
