...
import { loadUniqueFinishingRegister } from '@/actions/load';
...
const FormPress: React.FC<FormPressProps> = ({ id, tab }) => {
    ...
    const handleData = async (id: string) => {
        if (id?.length > 2 && handleEditForm.tab === tab) {
            await loadUniqueFinishingRegister(id)
                .then((response) => {
    ...
    const onSubmit = async (formData: PressFormValues) => {
        try {
            if (inspectionData?.id?.length > 0) {
                const res = await axios.post('/api/edit/finishing', formData);
                ...
                const res = await axios.post('/api/register/finishing', formData);
